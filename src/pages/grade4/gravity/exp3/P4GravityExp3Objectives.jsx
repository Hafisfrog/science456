...
export default function P4GravityExp3Objectives() {
  const navigate = useNavigate();
  const [lang, setLang] = useState("th");
  const audioRef = useRef(null);

  const BACK_PATH = "/p4/gravity";
  const NEXT_PATH = "/p4/gravity/exp3/skills";
  ...
  const content = useMemo(() => {
    return {
      th: {
        ...
        objectives: [
          "ศึกษาความแตกต่างของแรงดึงดูดระหว่างโลกกับดวงจันทร์",
          "เปรียบเทียบน้ำหนักของวัตถุชนิดเดียวกันเมื่ออยู่ในสถานที่ต่างกัน",
        ],
        back: "ย้อนกลับ",
        next: "ต่อไป",
        speak: "ฟัง",
      ...
      en: {
        ...
        objectives: [
          "Study the difference between Earth's gravity and the Moon's gravity.",
          "Compare the weight of the same object in different places.",
        ],
        back: "Back",
        next: "Next",
        speak: "Listen",
      ...
      ms: {
        ...
        section: "ตูยูแว เปิมเบอลาจาแร",
        objectives: [
          "จารี ปือเบซอแอ แร็ง บูมี ตาเระ บือนอ ดืองา แร็ง บูแล ตาเระ บือนอ",
          "บูเละฮ บาเน็ง บือระ บือนอ เฮาะ ซือรูปอ ยือนิฮ บีลอ โดะ ดี ตือปะ ลา-เอ็ง ลา-เอ็ง",
        ],
        back: "ฮูโนกือเละ",
        next: "ตือรุฮ",
        speak: "Dengar",
  ...
  return (
    <div className="relative h-screen w-screen overflow-hidden font-['Prompt',sans-serif]">
      ...
      <div className="relative h-full w-full p-6 max-[640px]:p-4">
        ...
        <main className="relative z-[3] ml-[48px] mt-[54px] max-w-[1120px] max-[1100px]:ml-[24px] max-[1100px]:mr-[240px] max-[1100px]:max-w-none max-[900px]:mr-[180px] max-[900px]:ml-4 max-[640px]:m-[28px_0_0]">
          <div className="mb-[28px] inline-block bg-[linear-gradient(90deg,#75d7e9,#2f61bf)] p-[10px] pr-[18px] shadow-[0_16px_28px_rgba(8,15,35,.28)]">
          ...
          <div className="space-y-[28px] max-[900px]:space-y-4">
            {t.objectives.map((objective, index) => (
              ...
              >
                ...
                <div className="flex-1 text-[30px] font-black leading-[1.24] text-black max-[1300px]:text-[25px] max-[900px]:text-[21px] max-[640px]:text-[17px]">
                  {objective}
                </div>
                <button
