  ...
  return (
    ...
    >
      ...
      <main className="relative z-10 mx-auto flex h-full w-full max-w-[1540px] flex-col items-center justify-center py-[clamp(8px,1.4vw,18px)]">
        ...
        <section className="mt-[clamp(14px,2vw,28px)] w-full max-w-[min(88vw,1180px)] max-[640px]:max-w-full">
          ...
          <div className="mx-auto grid w-full max-w-[1080px] grid-cols-1 gap-[clamp(12px,1.7vw,24px)] lg:grid-cols-2">
            {t.skills.map((skill, index) => (
              <div
                key={skill}
                className="flex min-w-0 items-center gap-[clamp(10px,1.2vw,18px)] rounded-[clamp(20px,2vw,32px)] border-[clamp(7px,.9vw,12px)] border-[#7caf44] bg-[#f2f5f4] px-[clamp(12px,1.4vw,22px)] py-[clamp(7px,.85vw,12px)] text-[clamp(19px,1.85vw,32px)] font-normal leading-tight text-black"
              >
      ...
      <div className="fixed bottom-3 left-1/2 z-20 flex -translate-x-1/2 items-center gap-2 sm:left-auto sm:right-3 sm:translate-x-0 sm:gap-3 md:bottom-7 md:right-7">
        <button
          ...
          onClick={() => navigate("/p6/electric-circuit/experiment-2/objectives")}
          type="button"
        >
          &laquo; {t.back}
        </button>
