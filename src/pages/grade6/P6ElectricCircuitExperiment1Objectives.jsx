...
const CONTENT = {
  th: {
    ...
    objectives: [
      "ศึกษาการต่อวงจรไฟฟ้าอย่างง่าย",
      "ศึกษาความสัมพันธ์ระหว่างจำนวนถ่านไฟฉายกับความสว่างของหลอดไฟ",
      "สังเกตผลของแหล่งกำเนิดไฟฟ้าต่อการทำงานของหลอดไฟ",
    ],
    back: "ย้อนกลับ",
    next: "ต่อไป",
  },
  en: {
    ...
    objectives: [
      "Study how to connect a simple electric circuit.",
      "Study the relationship between the number of batteries and bulb brightness.",
      "Observe how a power source affects the operation of a bulb.",
    ],
    back: "Back",
    next: "Next",
  },
    ...
    >
      ...
      <main className="relative z-10 mx-auto flex h-full w-full max-w-[1540px] flex-col items-center justify-center py-[clamp(8px,1.4vw,18px)]">
        ...
        <section className="mt-[clamp(14px,2vw,28px)] w-full max-w-[min(88vw,1320px)] max-[640px]:max-w-full">
          ...
          <div className="space-y-[clamp(12px,1.7vw,24px)]">
            {t.objectives.map((objective, index) => (
              ...
              >
                <div className="grid aspect-square w-[clamp(42px,4.2vw,58px)] place-items-center rounded-full bg-[#ee7447] text-[clamp(21px,2.2vw,29px)] font-black text-white">
                  {index + 1}
                </div>
                <div className="flex min-w-0 items-center gap-[clamp(10px,1.2vw,18px)] rounded-[clamp(20px,2vw,32px)] border-[clamp(7px,.9vw,12px)] border-[#7caf44] bg-[#f2f5f4] px-[clamp(12px,1.4vw,22px)] py-[clamp(7px,.85vw,12px)] text-[clamp(19px,1.85vw,32px)] font-normal leading-tight text-black max-[520px]:leading-[1.18]">
